...
class UpdateFallbackChecksumTest {
    ...
    fun `a fallback body whose hash mismatches the catalog is discarded, not staged`() {
        runBlocking {
            ...
            assertFalse(staged.exists(), "the mismatched body must not remain in the staging directory")
        }
    }
    ...
    fun `a fallback body whose hash matches the catalog is staged`() {
        runBlocking {
            ...
            assertEquals(expectedSha, sha256Of(staged))
        }
    }

    @Test
    fun `a fallback body with whitespace in hash is trimmed and verified`() {
        runBlocking {
            val servedFile = File.createTempFile("served-bytes-ws", ".bin", tempDir.toFile())
            servedFile.writeBytes(servedBytes)
            val rawSha = sha256Of(servedFile)
            val paddedSha = "  $rawSha  \n"
            ...
            assertNotNull(result, "a download with whitespace in sha256 must be trimmed and verified")
            val staged = File(result)
    ...
    @Test
    fun `a fallback body with uppercase hash is verified case-insensitively`() {
        runBlocking {
            val servedFile = File.createTempFile("served-bytes-upper", ".bin", tempDir.toFile())
            servedFile.writeBytes(servedBytes)
            val upperSha = sha256Of(servedFile).uppercase()
            ...
            assertNotNull(result, "a download with uppercase sha256 must be verified case-insensitively")
            val staged = File(result)
            assertTrue(staged.exists())
        }
    }
