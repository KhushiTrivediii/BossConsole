...
/**
 * End-to-end regression tests for the fallback-checksum fix (BossConsole#797).
 *
 * Where [UpdateFallbackChecksumTest] verifies the shared checksum gate directly, these
...
class UpdateServiceFallbackE2ETest {
    ...
    @Test
    fun `a fallback body stays unverified when the catalog carries blank sha256`() {
        servedBytes = goodBytes
        ...
        val path =
            runBlocking {
                service.downloadUpdate(catalogRow(version, assetName, goodBytes.size.toLong(), "   ")) {}
            }

        assertNotNull(path, "a blank hash catalog row must still download via the fallback")
        assertEquals(goodBytes.size.toLong(), File(path).length())
    }
}
