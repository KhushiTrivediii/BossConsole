...
class SpaceTerminalCommandsTest {
    ...
    ) = TabConfig(type = type, title = "t", initialCommand = command)
    ...
    @Test
    fun `case-insensitive terminal tab type is recognized`() {
        val layout =
            panel(
                "main",
                terminal("echo lower", type = "terminal"),
                terminal("echo upper", type = "TERMINAL"),
                terminal("echo mixed", type = "Terminal"),
            )

        assertEquals(
            listOf("echo lower", "echo upper", "echo mixed"),
            space(layout).terminalCommands(),
        )
    }
    ...
    @Test
    fun `commands with hidden display characters or BiDi overrides are rejected`() {
        assertEquals(
            SpaceLoadDisposition.REJECT,
        ...
        assertEquals(
            SpaceLoadDisposition.REJECT,
            spaceLoadDisposition(listOf("echo safe\u200Bzero"), requiresConfirmation = true),
        )
        assertEquals(
            SpaceLoadDisposition.REJECT,
            spaceLoadDisposition(listOf("echo safe\uD804\uDCBDwhoami"), requiresConfirmation = true),
        )
    }
