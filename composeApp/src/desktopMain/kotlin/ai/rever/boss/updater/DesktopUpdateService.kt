...
actual class UpdateService internal constructor(
    ...
    /**
     * Primary constructor used in production (matches expect declaration). Both
     * parameters take defaults in the internal constructor; this actual-constructor
     * signature matches `expect class UpdateService()` in commonMain so the
    ...
    private fun verifyDownloadedAsset(
        ...
        val cleanSha = sha256?.trim()?.takeIf { it.isNotBlank() }
        val actualSha = if (cleanSha != null) sha256Of(downloadFile) else null
        if (cleanSha != null && !cleanSha.equals(actualSha, ignoreCase = true)) {
            logger.error(
                LogCategory.SYSTEM,
                "Update checksum mismatch; discarding download",
                mapOf(
                    "asset" to assetName,
                    "expected" to cleanSha,
                    "actual" to (actualSha ?: ""),
                ),
            )
